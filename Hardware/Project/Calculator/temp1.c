...
void button_listener(int* button_x, int* button_y, int* time_lapsed){
    ...
    for(int k = 0; k < N_BUTTONS; k++){ // looping through columns
        CLR_BIT(PORTD, (k + 2)); // setting zero, i.e.

        for(int i = 0; i < N_BUTTONS; i++){
            if(!(PINC & (1 << (i)))){ // checking if that button is pressed
                *button_x = k;
                *button_y = i;
                *time_lapsed = 0;
                SET_BIT(PORTD, (k + 2));
...
int main(void){
    ...
    // Disable ADC (Magically turning analog pins to digital)
    ADCSRA &= ~(1 << ADEN);

    DDRC = 0x00;
    PORTC = 0xFF; // Setting input pull up
    
    // Digital Pins 8-13 of Arduinoare output (LCD)
    DDRB = 0xFF; // 1111.1111; set PB0-PB7 as outputs	 

    // Digital Pins 0-7 of Arduino are output (button grid)
    DDRD = 0xFF; // 0000.0000; writing to D0-D7
    PORTD = 0xFF;
