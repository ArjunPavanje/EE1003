...
void INSERT_INTO(char* buf, int index, char ch){
  char temp = buf[index];
...
void display_biline(int pos1, char* buf1, char* buf2){ // Displaying the input and output biline
  lcd_clear();
...
/* BUTTON FUNCTIONS */
void listen(int* button_x, int* button_y, int* debounce){
...
char jenson_button(int button_x, int button_y, int mode){
  ...
  if(mode) return jenson_button2[button_y][button_x];
  return jenson_button1[button_y][button_x];
}
...
int main(void){
  ...
  while(1){
    listen(&button_x, &button_y, &debounce);
    if(button_x != -1 && debounce == 0){
      ...
      if(token == '='){ // Return answer
        ...
        ans = evaluate(buf1);
        //eeprom_write_block((const void*) &ans, (void*) ADDRESS, sizeof(double));
      ...
      if(is_answer_loop){
        if(token != '<' && token != '(') clear_buf(&pos1, buf1);
        clear_buf(NULL, buf2);
        is_answer_loop = 0;
        braks = 0;
      }

      if(token == '<'){ // delete by one character
      ...
      else if(token == '_'){ //clearall
        clear_buf(&pos1, buf1);
        clear_buf(NULL, buf2);
      }
      else if(is_func(token)){
        INSERT_INTO(buf1, pos1, token);
        INSERT_INTO(buf1, pos1 + 1, '(');
        braks++;
        pos1 += 2;
      }
      else if(token == '&'){ //Shift
        ...
        if(pos1 > 0){
          char prev = buf1[pos1 - 1];
          if((prev == ')') || (prev >= '0' && prev <= '9') || (prev == 'M')) token = ')';
        }
    ...
    else if(!(PIND & (1 << (7))) && debounce > BUTTON_THRESHOLD && buf1[pos1 + 1] != '\0'){ // traversing left
      if(is_func(buf1[pos1])) pos1 += 2;
      else pos1 += 1;
      debounce = 0;
      display_biline(pos1, buf1, buf2);
    }
    else if(!(PINC & (1 << (5))) && debounce > BUTTON_THRESHOLD && pos1 != 0){ // traversing right
